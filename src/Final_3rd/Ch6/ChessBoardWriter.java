package Final_3rd.Ch6;

import javax.swing.*;
import java.awt.*;

public class ChessBoardWriter extends JPanel { // 체스보드 그리기
    private int number_of_rows;
    private int square_size;
    private int panel_width;
    private int offset = 20;

    /** Counstructor ChessBoardWriter 패널을 만들고 프레임을 씌움
     * @param rows 각 열변 칸의 갯수
     * @param size 한 칸의 길이 */
    ...
    public void paintBoard(int start_x, int start_y, int rows, int size, Graphics g) {
        for (int x = 0; x < rows; x++) {
            // loop invariant : x 열까지 그렸음 (x 증가 전)
            int x_position = start_x + x * size;
            for (int y = 0; y < rows; y++) {
                // loop invariant : x 열의 y 칸까지 그렸음 (x 증가 후, y 증가 전)
